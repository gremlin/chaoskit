      ...
      if (option.options) {
        return (
          <optgroup key={option.value} label={option.label}>
            {option.options.map(childOption => <option key={childOption.value} value={childOption.value}>{childOption.label}</option>)}
          </optgroup>
        );
      }
    ...
    return (
      <div className={classes}>
        ...
        <div className="form-select">
          <select disabled={disabled} id={this.id} name={name} value={selected} onChange={this.onChange}>
