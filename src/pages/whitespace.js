...
`.trim();
  ...
  return (
    <BaseLayout pageTitle="Whitespce">
      <p>
        We often need to apply margins or padding to individual elements. These
        utilities should be exclusively used for this type of spacing.
      </p>
      <p>
        To add space to any element we add a class of{' '}
        <code>
          .u-{'{type}'}
          {'{direction}'}--{'{amount}'}
        </code>
        . For example, if we wanted to add 32px of margin below an element, we
        would add a class of <code>.u-mb--large</code>.
      </p>

      <Alert type="warning" title="Note">
        <p>
          Do not use these modifiers to impact Grid, BlockGrid, or{' '}
          <a href="/inline/">Inline</a> spacing. They have their own spacing
      ...
      <Inline>
        <div>
          0px: <code>remove</code>
        </div>
        <div>
          4px: <code>xsmall</code>
        </div>
        <div>
          8px: <code>small</code>
        </div>
        <div>
          16px: <code>regular</code>
        </div>
        <div>
          24px: <code>medium</code>
        </div>
        <div>
          32px: <code>large</code>
        </div>
        <div>
          64px: <code>xlarge</code>
        </div>
      </Inline>

      <h3>Example</h3>

      <Live code={WhitespaceExample} scope={{}} />
    </BaseLayout>
  );
};
